...
# main_perclass.py
# ----------------
# Multi-configuration pipeline evaluation using ONLY the Custom Evaluator.
...
#     * NEW: Cross-class suppression (keeps highest-confidence label when handgun/knife overlap)
#     * Light geometry filters (min/max size, aspect ratio, edge/tiny-area)
# - Evaluation: Custom PipelineEvaluator (mAP50, mAP50-95, Precision, Recall, F1, TP/FP/FN)
# - Reports/plots: report_utils (Ultralytics metrics fully removed)
# - FLOPs: safe calculation with fallbacks (disable via evaluation.compute_flops)

# Visual debug:
# - Robust writer (config: evaluation.visual_debug_every, evaluation.visual_debug_limit)
# - NEW: On-frame legend with class colors.

# NOTE:
# - Keep FP16 ONLY on CUDA. On CPU we force FP32 for stability.
# - Paths/imports match your previous project layout.
# """
...
#                                 #frame_preds.extend(boxes_frame)
#                     #it += 1
...
#         # Lightweight visual debug writer: 1 of every N frames
...
NEW IN THIS VERSION:
- Integrated Privacy Protection Module
  * Face blurring (pixelate/gaussian) and silhouette masking
  * Selective application: "non_targets" (people without weapons) or "everyone"
  * Applied AFTER weapon detection to know who has weapons
...
Visual debug:
- Robust writer (config: evaluation.visual_debug_every, evaluation.visual_debug_limit)
- On-frame legend with class colors
- Privacy-protected output for debug images

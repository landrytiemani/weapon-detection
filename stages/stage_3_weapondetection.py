...
#         self.config = config
#         # self.model_name = self.config['approach']
#         # self.draw_enabled = self.config.get("draw", True)
#         # self.persons_with_weapons = set()

#         # # Load model based on approach
#         # if self.model_name == "rt_detr":
#         #     self.detector = RTDETRDetector(self.config['rt_detr'])
#         # elif self.model_name == "yolov8_efficientvit":
#         #     self.detector = YOLOv8EfficientViT(self.config.get('yolov8_efficientvit', {}))
#         # else:
#         #     raise ValueError(f"Unknown approach for Stage 3: {self.model_name}")

#         # Normalize approach once
...
#         elif self.model_name in ("yolov8_efficientvit", "yolov8_efficientvit"):  # allow hyphen alias if you want
...
#     def run(self, frame, person_data):
...
#             # Fixed square crop with person centered
...
#                 cv2.rectangle(annotated_frame, (x1, y1), (x2, y2), (255, 255, 0), 1)  # Draw Yellow box
...
#                     self.persons_with_weapons.add(person['id'])
...
#             # # Handle scaling from crop size to model input size (640x640)
#             # model_input_size = 640
#             # crop_h, crop_w = crop.shape[:2]
#             # scale_x = crop_w / model_input_size
#             # scale_y = crop_h / model_input_size

#             # if results and results[0].boxes:
#             #     for box, conf in zip(results[0].boxes.xyxy.cpu().numpy(), results[0].boxes.conf.cpu().numpy()):
#             #         # Convert from model (resized) space back to crop
#             #         wx1, wy1, wx2, wy2 = box
#             #         wx1 *= scale_x
#             #         wy1 *= scale_y
#             #         wx2 *= scale_x
...
#         Works for Ultralytics-style wrappers (may be nested at .model.model).
#         """
#         m = getattr(self.detector, "model", None)
#         if hasattr(m, "model"):  # e.g., Ultralytics
#             return m.model
#         return m
...
#                     opset=17,                       # RT-DETR needs >=16 due to grid_sampler
#                     use_half=False,
#                     cpu=True):
...
#         # RT-DETR usually returns [N, num_queries, 5] (cx, cy, w, h, conf) or similar;
#         # make num_queries dynamic so Netron shows flexible shapes.
#         dynamic_axes = {
#             "images":  {0: "batch"},
...
#             opset_version=opset,              # keep 17 (or 16+) for grid_sampler
...
================================
CORRECTED VERSION for PhD Research

Changes from original:
1. Added configurable crop_scale parameter (was hardcoded 1.2) for H1.3 experiments
2. Added batch inference capability for GPU efficiency
3. Added timing metrics for performance analysis
4. Fixed duplicate get_torch_model() method

Usage:
    python -c "from stage_3_weapondetection import WeaponDetectionStage; print('OK')"
"""

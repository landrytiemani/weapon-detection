# """
# FLOPs calculation utilities for model profiling.
# Supports YOLO models, PyTorch modules, and various profiling libraries.
# Fixed FP16/FP32 mismatch and parameter estimation formula.
# """
...
#     These are empirical values from official benchmarks.
#     """
#     total_params = sum(p.numel() for p in model.parameters()) / 1e6  # In millions
    
#     print(f"[FLOPS] Model has {total_params:.2f}M parameters")
    
#     # YOLOv8 models (at 640x640)
#     yolov8_estimates = {
#         'n': 8.7,    # YOLOv8n: 3.2M params, 8.7 GFLOPs
#         's': 28.6,   # YOLOv8s: 11.2M params, 28.6 GFLOPs
#         'm': 78.9,   # YOLOv8m: 25.9M params, 78.9 GFLOPs
#         'l': 165.2,  # YOLOv8l: 43.7M params, 165.2 GFLOPs
#         'x': 257.8   # YOLOv8x: 68.2M params, 257.8 GFLOPs
#     }
    
#     # RT-DETR models (at 640x640)
#     rtdetr_estimates = {
#         'small': 60,    # ~20M params
#         'medium': 92,   # ~30M params
#         'large': 136    # ~42M params
#     }
    
#     # Try to identify model type from parameter count
...
#             variant = 'm'
#         elif total_params < 55:
#             variant = 'l'
#         else:
#             variant = 'x'
        
#         base_gflops = yolov8_estimates[variant]
#         # Scale by input size (quadratic relationship)
#         scale_factor = (imgsz / 640) ** 2
#         estimated_gflops = base_gflops * scale_factor
        
#         print(f"[FLOPS] Identified as YOLOv8{variant} variant")
#         return estimated_gflops
    
#     # Check for RT-DETR
#     elif 'rtdetr' in model_name_lower or 'rt-detr' in model_name_lower or (15 <= total_params <= 45):
#         if total_params < 25:
#             variant = 'small'
#         elif total_params < 35:
#             variant = 'medium'
...
#         base_gflops = rtdetr_estimates[variant]
#         # Scale by input size
#         scale_factor = (imgsz / 640) ** 2
#         estimated_gflops = base_gflops * scale_factor
        
#         print(f"[FLOPS] Identified as RT-DETR {variant} variant")
#         return estimated_gflops
    
#     # Generic estimation for unknown models
#     # Rule of thumb: detection models are roughly 2.5-3.5 GFLOPs per million parameters at 640x640
#     else:
...
#         print(f"[FLOPS] Using generic estimation")
#         return estimated_gflops


# def compute_flops_gflops(model_or_yolo, imgsz=640, device=0, model_name="Model"):
#     """
#     Compute GFLOPs for models with improved accuracy and error handling.
...
#     print(f"[DEBUG] Model dtype: {original_dtype}, device: {original_device}")
    
#     # Ensure model is in eval mode
#     model = model.eval()
    
#     # Method 1: Try fvcore (most accurate)
#     try:
#         from fvcore.nn import FlopCountAnalysis
#         print(f"[FLOPS] Trying fvcore for {model_name}...")
        
#         # Convert model to FP32 for fvcore
...
#         if gflops > 0:
#             print(f"[FLOPS] {model_name} at {imgsz}px (fvcore): {gflops:.2f} GFLOPs")
#             return gflops
#         else:
#             raise ValueError("fvcore returned 0 GFLOPs")
            
#     except ImportError:
#         print(f"[INFO] fvcore not installed, skipping...")
#     except Exception as e:
#         print(f"[WARN] fvcore failed for {model_name}: {e}")
    
#     # Method 2: Try Ultralytics profiler (for YOLO models)
#     if is_yolo:
#         try:
#             print(f"[FLOPS] Trying ultralytics profiler for {model_name}...")
            
#             # Temporarily convert to FP32
#             model_fp32 = model.float() if original_dtype == torch.float16 else model
...
#                 # Try to get FLOPs from profile
#                 if hasattr(profile, 'flops'):
#                     gflops = float(profile.flops / 1e9)
...
#                     if gflops > 0:
#                         print(f"[FLOPS] {model_name} at {imgsz}px (ultralytics): {gflops:.2f} GFLOPs")
#                         return gflops
            
#         except Exception as e:
#             print(f"[WARN] Ultralytics profiler failed for {model_name}: {e}")
    
#     # Method 3: Try thop (fallback)
#     try:
#         from thop import profile
#         print(f"[FLOPS] Trying thop for {model_name}...")
        
#         # Convert model to FP32 for thop compatibility
#         model_fp32 = model.float()
#         model_copy = deepcopy(model_fp32).eval()
#         model_copy = strip_thop_buffers(model_copy)
        
#         # Create FP32 dummy input
#         dummy = torch.randn(1, 3, imgsz, imgsz, dtype=torch.float32, device=original_device)
...
#         if gflops > 0:
#             print(f"[FLOPS] {model_name} at {imgsz}px (thop): {gflops:.2f} GFLOPs")
#             return gflops
#         else:
#             raise ValueError("thop returned 0 GFLOPs")
            
#     except ImportError:
#         print(f"[INFO] thop not installed, skipping...")
#     except Exception as e:
#         print(f"[WARN] thop failed for {model_name}: {e}")
    
#     # Method 4: Model-specific estimation
#     try:
#         print(f"[FLOPS] Using model-specific estimation for {model_name}...")
#         estimated_gflops = get_model_specific_estimate(model, imgsz, model_name)
#         print(f"[FLOPS] {model_name} at {imgsz}px (estimated): {estimated_gflops:.2f} GFLOPs")
#         return estimated_gflops
        
#     except Exception as e:
#         print(f"[ERROR] All FLOPs calculation methods failed for {model_name}: {e}")
    
#     print(f"[ERROR] Could not calculate FLOPs for {model_name}")
#     return None
...
"""
FLOPs calculation utilities for model profiling - UPDATED VERSION
Properly handles measurement using consensus of fvcore and thop.
